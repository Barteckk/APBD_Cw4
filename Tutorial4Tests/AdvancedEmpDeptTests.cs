...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnFirstTwoHiredEmployees()
    {
        ...
        Assert.True(firstTwo.ElementAt(0).HireDate <= firstTwo.ElementAt(1).HireDate);
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, e => e.Mgr, e2 => e2.EmpNo, (e, e2) => new { Employee = e.EName, Manager = e2.EName });
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { e, d })
            .SelectMany(ed =>
                grades.Where(g => g.Losal <= ed.e.Sal && ed.e.Sal <= g.Hisal)
                    .Select(g => new { ed.e.EName, DName = ed.d.DName, g.Grade }));
        
        Assert.Contains(result, r => r.EName == "ALLEN" && r.DName == "SALES" && r.Grade == 3);
        Assert.DoesNotContain(result, r => r.Grade < 1 || r.Grade > 5);
    }
}
